...
const selectedMarkerDataKey = 'SELECTEDMARKERDATAKEY'
...
function markerStyle(features: FeatureLike) {
    ...
    featureList.forEach((feature) => {
        mode = Math.max(feature.get("mode"), mode);
    });
    if (size > 1) {
        return new Style({
            image: new Icon({
                crossOrigin: "anonymous",
                src: "data:image/svg+xml;utf8," + encodeURIComponent(greenClusterSvg),
        ...
        let feature = features.get("features")[0] as Feature;
...
export class OlMap extends CustomMap<Map> {
     ...
     */
    constructor(target: string | HTMLElement, forAddTraffic?: boolean, overlayEl?: HTMLDivElement) {
        super(new Map({
            target: target,
                ...
                switch (this.map.get(mapStateKey)) {
                    case mapState['NORMAL']:
                        if (features.length > 1) {
                            ...
                            if (selectedMarker.includes(features[0].getId() as number)) {
                                this.map.set(selectedMarkerDataKey, selectedMarker.filter(_ => _ !== features[0].getId()))
                            } else {
                                this.map.set(selectedMarkerDataKey, selectedMarker.concat(features[0].getId() as number))
                            }
                        }
                        break;
    ...
    registerBoxEndHandler(): void {
        this.dragBox.on("boxend", (e: DragBoxEvent) => {
            if (this.map.get(mapStateKey) === mapState['NORMAL']) {
                ...
                if (targetIds.every(_ => currentFeatureIds.includes(_))) {
                    this.selectedMarkerChangeCallback(currentFeatureIds.filter(_ => !targetIds.includes(_)))
                } else {
                    this.selectedMarkerChangeCallback(ArrayDeduplication(currentFeatureIds.concat(targetIds)))
                }
            }
        });
    ...
    addSelectedMarkerChangeEventCallback(callback: (target: SelectedMarkersType) => void): void {
        this.map.addChangeListener(selectedMarkerDataKey, () => {
            const currentSelectedMarkers: SelectedMarkersType = this.map.get(selectedMarkerDataKey)
            this.selectedMarkerChangeCallback(currentSelectedMarkers, true)
            if (callback) callback(currentSelectedMarkers)
        })
    }

    selectedMarkerChangeCallback(currentState: SelectedMarkersType, isEvent?: boolean) {
        ...
        if (!isEvent) this.map.set(selectedMarkerDataKey, currentSelectedMarkers)
    }
    ...
    clearPathLines():void {
        if(this.arrowVS) this.arrowVS.clear()
    }

    viewChangeById(cctvId: number): void {
        this.map.getView().fit(this.VS.getFeatureById(cctvId)?.getGeometry()?.getExtent()!)
    }

    addTrafficOverlayViewChangeListener(callback: (view: boolean) => void): void {
