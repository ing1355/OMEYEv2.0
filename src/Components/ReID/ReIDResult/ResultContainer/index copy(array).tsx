...
//                     {!_.resultList.find(__ => __.objectId === selectedTarget)?.timeAndCctvGroup.some(__ => Object.keys(__.results).some(___ => __.results[Number(___)].length > 0)) && <NoDataContainer>
//                         {
...
//                         }
//                     </NoDataContainer>}
//                     {_.resultList.find(__ => __.objectId === selectedTarget)?.timeAndCctvGroup.filter(__ => Object.keys(__.results).some(___ => __.results[Number(___)].length > 0)).map((__, _ind) => <TimeGroupContainer key={_ind}>
//                         <TimeGroupTitle>
//                             {convertFullTimeStringToHumanTimeFormat(__.startTime)} ~ {convertFullTimeStringToHumanTimeFormat(__.endTime)}
//                         </TimeGroupTitle>
//                         <TimeGroupContents>
//                             {
//                                 Object.keys(__.results).filter(___ => __.results[Number(___)].length > 0).map((___, __ind) => <TimeGroupCCTVRow key={__ind}>
//                                     <TimeGroupCCTVRowTitle>
//                                         <CCTVNameById cctvId={Number(___)} />
//                                     </TimeGroupCCTVRowTitle>
//                                     <TimeGroupCCTVRowContentsContainer>
//                                         {__.results[Number(___)].map((result, resultInd) => <TimeGroupCCTVItemBox key={resultInd} selected={selectedData && selectedData[selectedCondition] && selectedData[selectedCondition][selectedTarget] && selectedData[selectedCondition][selectedTarget].some(target => target.resultId === result.resultId) || false}>
//                                             <ItemMediasContainer>
//                                                 <ResultImageView data={{ ...result, cctvId: Number(___) }} type={_.resultList.find(r => r.objectId === selectedTarget)?.objectType!} />
//                                                 <ItemMediaContainer>
